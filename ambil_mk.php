...
// Proteksi Halaman: Hanya Mahasiswa yang boleh mengakses halaman KRS ini
...
// --- PROSES TOMBOL AMBIL MATA KULIAH ---
if (isset($_GET['aksi']) && $_GET['aksi'] == 'ambil') {
    $id_jadwal = mysqli_real_escape_string($koneksi, $_GET['id']);
    
    // Cek dulu apakah mata kuliah ini sudah diambil sebelumnya atau belum
    $cek = mysqli_query($koneksi, "SELECT * FROM krs WHERE username='$username_mhs' AND id_jadwal='$id_jadwal'");
    if (mysqli_num_rows($cek) == 0) {
        $insert = mysqli_query($koneksi, "INSERT INTO krs (username, id_jadwal) VALUES ('$username_mhs', '$id_jadwal')");
        if ($insert) {
            echo "<script>alert('Mata kuliah berhasil diambil!'); window.location.href='ambil_mk.php';</script>";
        }
    } else {
        echo "<script>alert('Mata kuliah ini sudah kamu ambil sebelumnya!'); window.location.href='ambil_mk.php';</script>";
    }
}

// --- PROSES TOMBOL BATALKAN MATA KULIAH ---
if (isset($_GET['aksi']) && $_GET['aksi'] == 'batal') {
    ...
    if ($hapus) {
        echo "<script>alert('Pengambilan mata kuliah berhasil dibatalkan.'); window.location.href='ambil_mk.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Penawaran RPS - KRS Mahasiswa</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="ambil_mk.php" class="active"><i class="bi bi-journal-plus"></i> Penawaran RPS</a>
    ...
    <div class="main-content">
        <div class="card shadow-sm p-4 mb-4 border-0 bg-white" style="border-left: 5px solid #0d6efd !important;">
            <h2 class="text-primary fw-bold mb-1">KRS & Penawaran RPS Mata Kuliah</h2>
            <p class="text-muted mb-0">Silakan pilih atau batalkan jadwal mata kuliah yang ingin kamu programkan semester ini.</p>
        </div>
                    ...
                    <table class="table table-hover align-middle">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th>Ruangan</th>
                                <th class="text-center" style="width: 150px;">Status Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            // Query memanggil seluruh jadwal kuliah
                            ...
                            if(mysqli_num_rows($query) == 0) {
                                echo "<tr><td colspan='8' class='text-center text-muted py-4'>Belum ada penawaran mata kuliah tersedia.</td></tr>";
                            } else {
                                while($data = mysqli_fetch_array($query)){
                                    $id_jd = $data['id_jadwal'];
                                    
                                    // Cek apakah mahasiswa ini sudah mengambil jadwal ini atau belum
                                    $cek_krs = mysqli_query($koneksi, "SELECT * FROM krs WHERE username='$username_mhs' AND id_jadwal='$id_jd'");
                                    $sudah_ambil = (mysqli_num_rows($cek_krs) > 0);
                                ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    ...
                                    <td class="text-center">
                                        <?php if ($sudah_ambil) : ?>
                                            <a href="ambil_mk.php?aksi=batal&id=<?= $data['id_jadwal']; ?>" class="btn btn-danger btn-sm w-100" onclick="return confirm('Batalkan pengambilan mata kuliah ini?')">
                                                <i class="bi bi-x-circle"></i> Batalkan
                                            </a>
                                        <?php else : ?>
                                            <a href="ambil_mk.php?aksi=ambil&id=<?= $data['id_jadwal']; ?>" class="btn btn-success btn-sm w-100">
